...
/* -------------------------------------------------------------------------
 * Android-specific open: uses an fd from Android's UsbManager instead of
 * enumerating USB devices (which requires udev and doesn't work on Android).
 *
 * The fd comes from UsbDeviceConnection.getFileDescriptor() and is passed
 * to libusb_wrap_sys_device() to get a libusb_device_handle.
 *
 * errmsg/errmsg_len: caller-supplied buffer for a human-readable failure
 * description.  Always NUL-terminated on return.
 *
 * This function lives here (not in a separate file) so it has access to the
 * private static helpers: rtlsdr_init_baseband, rtlsdr_set_i2c_repeater,
 * rtlsdr_i2c_read_reg, rtlsdr_check_dongle_model, etc.
 * ------------------------------------------------------------------------- */
                        ...
                        char *errmsg, size_t errmsg_len)
{
	...
	dev = malloc(sizeof(rtlsdr_dev_t));
	if (!dev) {
		if (errmsg) snprintf(errmsg, errmsg_len, "malloc failed");
		return -ENOMEM;
	}
	...
	/* LIBUSB_OPTION_NO_DEVICE_DISCOVERY must be passed at context-creation time
	 * via libusb_init_context() — calling libusb_set_option() after
	 * libusb_init() is too late because init already attempts to enumerate
	 * /dev/bus/usb/ which fails with LIBUSB_ERROR_IO on Android without root. */
	{
		const struct libusb_init_option init_opts[] = {
			{ LIBUSB_OPTION_NO_DEVICE_DISCOVERY, { 0 } }
		};
		r = libusb_init_context(&dev->ctx, init_opts, 1);
	}
	if (r < 0) {
		if (errmsg)
			snprintf(errmsg, errmsg_len,
			         "libusb_init_context failed: %d (%s)", r, libusb_strerror(r));
		free(dev);
		return r;
	}
	...
	/* Wrap the Android-granted fd into a libusb device handle */
	r = libusb_wrap_sys_device(dev->ctx, (intptr_t)fd, &dev->devh);
	if (r < 0) {
		if (errmsg)
			snprintf(errmsg, errmsg_len,
			         "libusb_wrap_sys_device(fd=%d) failed: %d (%s)",
			         fd, r, libusb_strerror(r));
		goto err;
	}

	/* Claim interface 0.
	 * Android already ran USBDEVFS_CLAIMINTERFACE on this fd from the Java
	 * side, so LIBUSB_ERROR_BUSY (-6) simply means "already ours" — safe to
	 * continue.  Any other error is a real failure. */
	r = libusb_claim_interface(dev->devh, 0);
	if (r < 0 && r != LIBUSB_ERROR_BUSY) {
		if (errmsg)
			snprintf(errmsg, errmsg_len,
			         "libusb_claim_interface failed: %d (%s)",
			         r, libusb_strerror(r));
		goto err;
	}
	...
	/* Dummy write to confirm the USB control pipe is working.
	 * On Android we must NOT call libusb_reset_device() — it would revoke
	 * Android's USB grant and leave the fd unusable. Log and continue. */
	if (rtlsdr_write_reg(dev, USBB, USB_SYSCTL, 0x09, 1) < 0) {
		fprintf(stderr, "rtlsdr_open_android: initial USB write failed"
	...
	/* Initialise GPIOs and reset tuner before remaining probes */
	rtlsdr_set_gpio_output(dev, 4);
	rtlsdr_set_gpio_bit(dev, 4, 1);
	rtlsdr_set_gpio_bit(dev, 4, 0);
